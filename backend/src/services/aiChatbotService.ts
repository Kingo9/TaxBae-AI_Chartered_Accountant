...
export class AIChatbotService {
  ...
  constructor() {
    ...
    // Indian Tax Law Knowledge Base (RAG data)
  ...
  private async getUserContext(userId: string): Promise<string> {
    try {
      // Get user's recent transactions and investments
...
Remember: You're helping users save tax legally and plan their finances better. Be friendly, professional, and trustworthy.`;
  ...
  async generateResponse(
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        model: 'gpt-4', // Use GPT-4 for better tax advice
  ...
  // Get quick tax tips based on current date
